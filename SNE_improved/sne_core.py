...
from sne_semantic import SemanticNetwork
...
class CreativeIntentionGenerator:
    ...
    def generate_intentions(self, state: np.ndarray, focus: AttentionFocus,
        ...
        self._update_emotional_state(experiences)
        self._update_semantic_network(state, experiences)
        ...
        if best_action is not None:
            ...
            if len(experiences) > 20:
                for _ in range(2):
                    additional_exploit_intent = self._generate_exploitation_intentions(
                        best_action, focus
                    )[0]
                    intentions.append(additional_exploit_intent)
        ...
        semantic_intents = self._generate_semantic_intentions(state, focus)
        intentions.extend(semantic_intents)
        
        if len(self.success_patterns) > 2:
            success_based_intents = self._generate_success_based_intentions(focus)
            intentions.extend(success_based_intents)
        ...
        emotional_intents = self._generate_emotional_intentions(focus)
        intentions.extend(emotional_intents)
        ...
        for intent in intentions:
            ...
            emotional_bonus = self._get_emotional_bonus(intent)
            
            score = (
                0.40 * intent.confidence +
                0.25 * intent.novelty * novelty_bonus +
                0.20 * (1 - intent.fuzziness) +
                0.10 * emotional_bonus +
                0.05 * random.uniform(0, 1)
            )
            
            if best_action is not None and intent.intention_type == IntentionType.MODIFICATION:
                score *= 1.2
    ...
    def _generate_analogical_intentions(self, state: np.ndarray, focus: AttentionFocus,
        ...
        if distances[similar_idx] < 1.0:
            ...
            intent = MetaIntention(
                ...
                expected_outcome=state * 0.5 + analogical_action * 0.5,
    ...
    def _update_emotional_state(self, experiences: List[Experience]):
        if not experiences:
            return
    ...
    def _update_semantic_network(self, state: np.ndarray, experiences: List[Experience]):
        if not experiences:
            return
        
        recent_experience = experiences[-1]
        self.semantic_network.update_associations_from_experience(
            state=state,
            action=recent_experience.action,
            reward=recent_experience.reward,
            context={"emotional_state": self.emotional_state.value}
        )
    
    def _generate_semantic_intentions(self, state: np.ndarray, focus: AttentionFocus) -> List[MetaIntention]:
        intentions = []
        
        state_concepts = [f"state_dim_{i}_{state[i]:.2f}" for i in range(len(state))]
        for concept in state_concepts:
            self.semantic_network.activate_concept(concept, 1.0)
        
        self.semantic_network.spread_activation(steps=3)
        active_concepts = self.semantic_network.get_active_concepts(threshold=0.4)
        
        if active_concepts:
            retrieved_memories = self.semantic_network.retrieve_from_long_term_memory(
                cue=state_concepts[0],
                max_results=3
            )
            
            if retrieved_memories:
                best_memory = max(retrieved_memories, key=lambda x: x.importance)
                if hasattr(best_memory, 'content') and 'action' in best_memory.content:
                    semantic_action = best_memory.content['action'].copy()
                    confidence = min(0.7, best_memory.importance * 0.8)
                    
                    intent = MetaIntention(
                        intention_type=IntentionType.ANALOGICAL,
                        base_action=semantic_action,
                        target_feature="semantic",
                        confidence=confidence,
                        novelty=0.6,
                        fuzziness=0.3,
                        expected_outcome=semantic_action,
                        explanation_confidence=confidence * 0.9,
                        causal_hypothesis=f"Semantic association from memory with importance {best_memory.importance:.2f}",
                        abandonment_threshold=0.4
                    )
                    intentions.append(intent)
            
            if len(intentions) == 0:
                semantic_action = np.random.uniform(-1, 1, self.action_dim)
                
                intent = MetaIntention(
                    intention_type=IntentionType.ANALOGICAL,
                    base_action=semantic_action.copy(),
                    target_feature="semantic",
                    confidence=0.5,
                    novelty=0.8,
                    fuzziness=0.5,
                    expected_outcome=semantic_action,
                    explanation_confidence=0.4,
                    causal_hypothesis=f"Semantic association from {len(active_concepts)} concepts",
                    abandonment_threshold=0.5
                )
                intentions.append(intent)
    ...
    def _generate_emotional_intentions(self, focus: AttentionFocus) -> List[MetaIntention]:
        intentions = []
        
        if self.emotional_state == EmotionalState.POSITIVE:
            action = np.random.uniform(0, 1, self.action_dim)
            intent = MetaIntention(
                intention_type=IntentionType.DIRECTIONAL,
                base_action=action.copy(),
                target_feature="emotional",
                confidence=0.6,
                novelty=0.5,
                fuzziness=0.3,
                expected_outcome=action,
                explanation_confidence=0.5,
                causal_hypothesis="Positive emotional bias",
                abandonment_threshold=0.4
            )
            intentions.append(intent)
        elif self.emotional_state == EmotionalState.NEGATIVE:
            action = np.random.uniform(-1, 0, self.action_dim)
            intent = MetaIntention(
                intention_type=IntentionType.COUNTERFACTUAL,
                base_action=action.copy(),
                target_feature="emotional",
                ...
                expected_outcome=action,
                explanation_confidence=0.3,
                causal_hypothesis="Negative emotional bias",
                abandonment_threshold=0.6
            )
            intentions.append(intent)
        elif self.emotional_state == EmotionalState.CURIOUS:
            action = np.random.uniform(-1, 1, self.action_dim) * 1.5
            ...
            intent = MetaIntention(
                ...
                fuzziness=0.8,
                expected_outcome=action,
                explanation_confidence=0.2,
                causal_hypothesis="Curiosity-driven exploration",
    ...
    def _get_emotional_bonus(self, intent: MetaIntention) -> float:
        bonus = 1.0
        
        if self.emotional_state == EmotionalState.POSITIVE:
            if intent.intention_type == IntentionType.MODIFICATION:
                bonus = 1.2
        elif self.emotional_state == EmotionalState.NEGATIVE:
            if intent.intention_type == IntentionType.COUNTERFACTUAL:
                bonus = 1.2
        elif self.emotional_state == EmotionalState.CURIOUS:
            if intent.intention_type == IntentionType.RANDOM:
                bonus = 1.3
        elif self.emotional_state == EmotionalState.EXCITED:
            if intent.intention_type == IntentionType.COMBINATION:
                bonus = 1.2
        
        return bonus
    
    def _generate_success_based_intentions(self, focus: AttentionFocus) -> List[MetaIntention]:
        intentions = []
        
        if len(self.success_patterns) < 2:
            return intentions
        
        best_success = max(self.success_patterns, key=lambda x: x[2])
        best_state, best_action, best_reward = best_success
        ...
        for _ in range(2):
            ...
            intent = MetaIntention(
                intention_type=IntentionType.MODIFICATION,
                base_action=best_action.copy(),
                target_feature="success_based",
                confidence=0.8 - magnitude,
                novelty=0.3 + magnitude * 0.5,
                fuzziness=magnitude,
                expected_outcome=modified_action,
                explanation_confidence=0.7,
                causal_hypothesis=f"Based on successful action (reward: {best_reward:.2f})",
                abandonment_threshold=0.3
            )
            intentions.append(intent)

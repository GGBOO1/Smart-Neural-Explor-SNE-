...
class CreativeIntentionGenerator:
    ...
    def generate_intentions(self, state: np.ndarray, focus: AttentionFocus,
        ...
        for intent in intentions:
            ...
            score = (
                0.30 * intent.confidence +
                0.30 * intent.novelty * novelty_bonus +
                0.20 * (1 - intent.fuzziness) +
                0.15 * emotional_bonus +
    ...
    def _generate_semantic_intentions(self, state: np.ndarray, focus: AttentionFocus) -> List[MetaIntention]:
        ...
        for concept in state_concepts:
            self.semantic_network.activate_concept(concept, 0.8)
        
        self.semantic_network.spread_activation(steps=2)
        active_concepts = self.semantic_network.get_active_concepts(threshold=0.3)
